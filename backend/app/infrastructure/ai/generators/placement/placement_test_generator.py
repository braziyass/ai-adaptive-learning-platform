...
from app.infrastructure.ai.types import GenerationRequest, GeneratedContent, RetrievedChunk


class PlacementTestGenerator(BaseGenerator):
    def generate(self, request: GenerationRequest, chunks: list[RetrievedChunk] | None = None) -> GeneratedContent:
        if chunks is None:
